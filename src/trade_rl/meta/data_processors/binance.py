...
class Binance(_Base):
    ...
    def download_data(self, ticker_list: List[str]):
        ...
        if self.time_interval == "1s":
            ...
            self.limit = 1000
            final_df = self.fetch_n_combine(
                self.start_date, self.end_date, ticker_list)
        else:
    ...
    def dataframe_with_limit(self, symbol):
        ...
        while True:
            ...
            if self.time_diff == None:
                self.time_diff = new_df.loc[1]["datetime"] - \
                    new_df.loc[0]["datetime"]
        ...
        for current, symbol in enumerate(symbols):
            ...
            for date in dates:
                ...
                if current_date >= start_date and current_date <= end_date:
                    path = self.get_path(
                        trading_type, "aggTrades", "daily", symbol)
                    file_name = f"{symbol.upper()}-aggTrades-{date}.zip"
                    fhandle = self.download_n_unzip_file(
                        path, file_name, date_range)
                    map[symbol] += fhandle
        return map
        ...
        for tic in map.keys():
            ...
            for i, csv in enumerate(map[tic]):
                ...
                if i == 0:
                    ...
                    nextday_str = (day_dt + dt.timedelta(1)
                                   ).strftime("%Y-%m-%d")
                    ...
                    if dailyfinal.shape[0] != 86400:
                        raise ValueError(
                            "everyday should have 86400 datapoints")
