...
class Ccxt(_Base):
    def __init__(
        ...
        super().__init__("ccxt", start_date, end_date, time_interval)
        ...
        for ticker in ticker_list:
            ...
            if first_time:
                dataset = pd.DataFrame(
                    columns=crypto_column, index=df["time"].values)
                first_time = False
            temp_col = pd.MultiIndex.from_product(
                [[ticker], ["open", "high", "low", "close", "volume"]]
            )
            dataset[temp_col] = df[["open", "high",
    ...
    def df_to_ary(self, pair_list, tech_indicator_list=None):
        ...
        price_array = df[pd.MultiIndex.from_product(
            [pair_list, ["close"]])].values
        tech_array = df[
